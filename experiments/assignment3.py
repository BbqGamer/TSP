...
@njit(cache=True)
def random_start_greedy_experiment(n, sol_size, D, intra_node: bool) -> np.ndarray:
    np.random.seed(42)

    best_score = np.inf
    for i in range(200):
        start_sol, unselected = random_starting(n, sol_size)
        sol = local_search_steepest(start_sol, unselected, D, intra_node)
...
if __name__ == "__main__":
    ...
    best_sol = random_start_greedy_experiment(
        len(problem), problem.solution_size, problem.D, True)
    problem.visualize(best_sol)

    best_sol = random_start_greedy_experiment(
        len(problem), problem.solution_size, problem.D, False)
    problem.visualize(best_sol)
